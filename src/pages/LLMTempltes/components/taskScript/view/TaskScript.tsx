...
import useTaskScriptViewModel from '../viewModel/useTaskScriptViewModel';

const TaskScript = () => {
  const { t, i18n } = useTranslation(['llm', 'common']);

  const contentDefaultParams = {
    topic: '',
    content: '',
  ...
  const methods = useForm<LlmScriptContentProps>({
    mode: 'onSubmit',
    resolver: yupResolver(DataSchema),
    defaultValues: contentDefaultParams,
  });
      ...
      <div className="mt-8 wf-form ">
        <div className="flex flex-row w-full gap-20 mt-6 laptop:flex-col ">
          <FormProvider {...methods}>
            <form className="w-1/2 p-10 bg-white rounded-xl desktop-sm:w-full" onSubmit={handleSubmit(onSubmit)}>
              <ul>
                <li>
                  <InputTextParagraph
                    helperText={t('llm:영상_주제를_입력해주세요') as string}
                    registerName={'topic'}
                    setContent={setTopic}
                    content={topic}
                    bigName={t('llm:영상_주제')}
                ...
                <li className="mt-10">
                  <InputTextParagraph
                    helperText={t('llm:포함할_내용을_입력하세요') as string}
          ...
          <form className="flex flex-col w-1/2 p-10 bg-white rounded-xl desktop-sm:w-full">
            ...
            <div className="flex justify-end mt-8 ">
              <DownloadFileBtn
                fileTitleContent={createdContents}
                fileContentContent={createdContents}
                name="script"
                disabled={disableBtn}
              />
            </div>
