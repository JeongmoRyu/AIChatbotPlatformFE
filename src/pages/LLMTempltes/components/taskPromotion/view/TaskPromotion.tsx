...
import useTaskPromotionViewModel from '../viewModel/useTaskPromotionViewModel';

function TaskPromotion() {
  ...
  const DataSchema = yup.object().shape({
    ...
    detailedService: yup.string().required(),
    companyName: yup.string().required(),
    keyword: yup.string().required(),
    target: yup.string().required(),
  });

  const methods = useForm<LlmPromotionProps>({
    mode: 'onSubmit',
  ...
  const {
    ...
    isReCreate,
    disableBtn,
  } = useTaskPromotionViewModel({ setValue });

  return (
    <div className="contents ai-service max-w-[1920px] p-16 desktop-sm:max-w-[880px] laptop:mt-16 ">
      <div className="flex flex-row items-center">
        <p className="text-xl font-bold">{t('llm:홍보_문구_생성')}</p>
      </div>
      <p className="py-6 border-b text-primary-gray border-set-border">{t('llm:원하는_주제_및_키워드를_입력하세요')}</p>
      ...
      <div className="mt-8 wf-form ">
        <div className="flex flex-row w-full gap-20 mt-6 laptop:flex-col ">
            ...
            >
              <ul>
                {inputFields.map((item) => (
                  <li key={item.name}>
                    <InputTextParagraph
                      helperText={t(`llm:${item.helperTextKey}`) || ''}
                      registerName={item.name}
                      setContent={item.setContent}
                      content={contentInput}
                      bigName={t(`llm:${item.bigNameKey}`)}
                      type={'input'}
                      point={item.helperTextKey ? true : false}
          ...
          <form className="flex flex-col w-1/2 p-10 bg-white rounded-xl desktop-sm:w-full">
            <ResultCard
              title={t('llm:홍보_문구')}
              createdSentence={createdPromotion}
            ...
            <div className="flex justify-end mt-8 ">
              <DownloadFileBtn fileContentContent={createdPromotion} name="promotion" disabled={disableBtn} />
            </div>
          </form>
        </div>
      </div>
